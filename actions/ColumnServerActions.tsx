...
export async function handleCreateColumn(data: ColumnCreationData) {
  ...
  if (!parse.success) {
    return {
      success: false,
      message: "Failed to create column due to validation error",
    };
  }
...
export async function handleDeleteColumn(data: ColumnDeletionData) {
  ...
  if (!parse.success) {
    return {
      success: false,
      message: "Failed to delete column due to validation error",
    };
  }
...
export async function handleDeleteColumnTasks({
  columnId,
  boardId,
}: {
  columnId: string;
