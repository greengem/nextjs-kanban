    ...
    <div className="flex gap-3 p-5">
      ...
      <div className="flex-col w-full pr-5">
        ...
        <div className="text-sm font-normal">
          <p>
            Created on{" "}
            {format(new Date(selectedTask.createdAt), "MMMM d, yyyy")} | Updated
            on {format(new Date(selectedTask.updatedAt), "MMMM d, yyyy")}
          </p>
          <p>
            In column{" "}
            <span className="text-primary">{selectedTask.column.title}</span>
          </p>
        </div>
      </div>
    </div>
